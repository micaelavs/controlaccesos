...
$content = '<!DOCTYPE html>
			<html lang="en">
			<head>
			<link rel="stylesheet" href="' . $css_pdf . '">
			<meta charset="utf-8">';
		...
		foreach ($lista_contrato as $value) {
				...
				$content .= '<div>
							<table> 
								<tr>
									...
									<td colspan="3" style="text-align:right;"><img src="'.$logo.'" height="60"></td>
								</tr>
							</table>
			...
			if ($diff->days > 1) {
				for ($i = 1; $i < $diff->days; $i++) {
					...
					$habil = true; //Informacion_fecha::es_habil( new DateTime($fecha_suma->format('Y-m-d')) );
